    ...
    <div class = "tabla_gestion">
        <table>
          ...
          <?php
            ...
            $correo_user = $_SESSION['sesion'][0];
            $busqueda = mysqli_query($conexion, "SELECT codigo, correo, dia, mes, ano, cantidad_personas, primer_nombre, primer_apellido, paquete_codigo, factura.estatus FROM factura INNER JOIN cliente WHERE cliente.cedula_rif = factura.cliente_cedula_rif AND cliente.correo = '$correo_user'"); 
      ...
      <?php
        require_once('Conexion.php');
        $busqueda_cedula = mysqli_query($conexion, "SELECT cedula_rif FROM cliente WHERE correo = '$correo_user'");
        $cedula_rif = 0;

        foreach ($busqueda_cedula as $i) {
          $cedula_rif = $i['cedula_rif'];
        }

        $busqueda = mysqli_query($conexion, "SELECT COUNT(codigo) FROM factura WHERE estatus = 0 AND cliente_cedula_rif = $cedula_rif"); 
        $eliminadas = 0;
